...
export class PdfService {
  ...
  private collect(doc: any): Buffer {
    const chunks: Buffer[] = [];
  ...
  private headerBlock(doc: any, title: string, docNo: string, docDate: string, status: string) {
    doc.fontSize(18).font('Helvetica-Bold').text('Kosovo ERP System', { align: 'center' });
    doc.moveDown(0.3);
    doc.fontSize(13).font('Helvetica-Bold').text(title, { align: 'center' });
    doc.moveDown(0.2);
    doc.fontSize(10).font('Helvetica').text(`Doc No: ${docNo}   |   Date: ${new Date(docDate).toLocaleDateString('en-GB')}   |   Status: ${status}`, { align: 'center' });
    doc.moveDown(0.8);
  ...
  private partyRow(doc: any, leftLabel: string, leftEntity: Record<string, any>, rightLabel: string, rightValue: string) {
    ...
    if (leftEntity.fiscalNo) doc.text(`Fiscal: ${leftEntity.fiscalNo}`, col1, doc.y);
    if (leftEntity.vatNo) doc.text(`VAT: ${leftEntity.vatNo}`, col1, doc.y);
  ...
  private linesTableWithDiscount(doc: any, lines: any[]) {
    ...
    const headers = ['#', 'Item', 'Qty', 'Price', 'D%', 'Disc', 'Net', 'Tax', 'Gross'];
  ...
  private linesTableNoDiscount(doc: any, lines: any[]) {
    ...
    const headers = ['#', 'Item', 'Qty', 'Price', 'Net', 'Tax', 'Gross'];
    const widths = Object.values(cols);
    let x = 50;
  ...
  private totalsBlock(doc: any, subtotal: number, taxTotal: number, grandTotal: number, discountTotal?: number) {
    ...
    doc.text('Grand Total:', lx, doc.y, { width: 90 });
  ...
  private footer(doc: any) {
    doc.font('Helvetica').fontSize(8).fillColor('#aaaaaa')
      .text('Generated by Kosovo ERP System', 50, doc.y, { align: 'center' });
    doc.fillColor('#000000');
  }

  generatePurchaseInvoicePdf(invoice: any): Buffer {
    const doc = this.buildDoc();

    this.headerBlock(doc, 'PURCHASE INVOICE', invoice.docNo, invoice.docDate, invoice.status);

    if (invoice.supplierInvoiceNo) {
      doc.font('Helvetica').fontSize(9).text(`Supplier Inv. No: ${invoice.supplierInvoiceNo}`);
      doc.moveDown(0.4);
    }

    this.partyRow(doc, 'Supplier', invoice.supplier ?? {}, 'Warehouse', invoice.warehouse?.name ?? '-');
  ...
  generateSalesInvoicePdf(invoice: any): Buffer {
    const doc = this.buildDoc();

    this.headerBlock(doc, 'SALES INVOICE', invoice.docNo, invoice.docDate, invoice.status);

    if (invoice.paymentMethod) {
      doc.font('Helvetica').fontSize(9).text(`Payment Method: ${invoice.paymentMethod.name}`);
      doc.moveDown(0.4);
    }

    this.partyRow(doc, 'Customer', invoice.customer ?? {}, 'Warehouse', invoice.warehouse?.name ?? '-');
    ...
    if (invoice.fiscalReference) {
      doc.font('Helvetica').fontSize(9).text(`Fiscal Reference: ${invoice.fiscalReference}`);
      doc.moveDown(0.5);
    }
  ...
  generateSalesReturnPdf(ret: any): Buffer {
    const doc = this.buildDoc();

    this.headerBlock(doc, 'SALES RETURN', ret.docNo, ret.docDate, ret.status);

    doc.font('Helvetica').fontSize(9).text(`Original Invoice: ${ret.salesInvoice?.docNo ?? '-'}`);
    if (ret.reason) doc.text(`Reason: ${ret.reason}`);
    doc.moveDown(0.6);

    this.partyRow(doc, 'Customer', ret.customer ?? {}, 'Original Invoice', ret.salesInvoice?.docNo ?? '-');
