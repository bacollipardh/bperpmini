...
export function PurchaseInvoiceForm({
  ...
  async function onSubmit(e: FormEvent) {
    e.preventDefault();
    // Client-side guard: all lines must have an item selected
      ...
      <div className="rounded-2xl border bg-white p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          <div>
            ...
            />
            <p className="text-xs text-slate-400 mt-1">Defines the document number format (e.g. PI-2025/0001)</p>
          </div>
          <SelectInput label="Supplier" value={form.supplierId} onChange={(value) => setForm({ ...form, supplierId: value })} options={suppliers.map((x) => ({ value: x.id, label: x.name }))} />
          ...
          <TextInput label="Supplier Invoice No" value={form.supplierInvoiceNo} onChange={(e) => setForm({ ...form, supplierInvoiceNo: e.target.value })} />
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          {mode === 'edit' && data?.status === 'DRAFT' ? (
            <ConfirmButton label="Post Document" confirmText="Post this purchase invoice?" onClick={onPost} className="rounded-xl bg-emerald-600 text-white px-4 py-2 text-sm font-medium" />
          ) : null}
        </div>
        <FormActions submitLabel={mode === 'create' ? 'Create Purchase Invoice' : 'Update Purchase Invoice'} busy={busy} />
      </div>
    </form>
  );
}
